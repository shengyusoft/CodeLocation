...
public class LoanApprovalRegisterController extends BaseController {
	...
	public String listPage(HttpServletRequest request) {
		...
		return "/basic/loanApprovalRegister/list";
	}
	/**
	 * 总经理审批列表页面
	 * @param request
	 ...
	 * @return
	 */
	@RequestMapping("/managerZJL")
	public String listPageZJL(HttpServletRequest request) {
		
		return "/basic/loanApprovalRegister/listZJL";
	}
	/**
	 * 出纳审批列表页面
	 * @param request
	 ...
	 * @return
	 */
	@RequestMapping("/managerCN")
	public String listPageCN(HttpServletRequest request) {
		
		return "/basic/loanApprovalRegister/listCN";
	}
	/**
	 * 新增页面
	 * @param request
	...
	public String addPage(HttpServletRequest request) {
		return "/basic/loanApprovalRegister/add";
	}
	/**
	 * 编辑页面
	 * @param request
	...
	public String editPage(HttpServletRequest request, Long id) {
		...
		return "/basic/loanApprovalRegister/edit";
	}
	/**
	 * 总经理审批页面
	 * @param request
	 * @return
	 */
	@RequestMapping("/approvalZJLPage")
	public String approvalZJLPage(HttpServletRequest request, Long id) {
		request.setAttribute("id", id);
		return "/basic/loanApprovalRegister/approvalZJL";
	}
	/**
	 * 出纳审批页面
	 * @param request
	 * @return
	 */
	@RequestMapping("/approvalCNPage")
	public String approvalCNPage(HttpServletRequest request, Long id) {
		SessionInfo sessionInfo = 
				(SessionInfo) request.getSession().getAttribute(GlobalConstant.SESSION_INFO);
		String name = sessionInfo.getName();
		request.setAttribute("name", name);
		request.setAttribute("id", id);
		return "/basic/loanApprovalRegister/approvalCN";
	}
	/**
	 * 审批页面
	 * @param request
	...
	public String approvalPage(HttpServletRequest request, Long id, Long processId) {
		...
		return "/basic/loanApprovalRegister/approval";
	}
	/**
	 * 详情页面
	 * @param request
	...
	public String detailPage(HttpServletRequest request, Long id) {
		request.setAttribute("id", id);
		return "/basic/loanApprovalRegister/detail";
	}
	...
	@ResponseBody
	public Grid dataGrid(LoanApprovalRegister vo, PageFilter ph, HttpServletRequest request) {
		Grid grid = new Grid();
...
//				vo.setApplierId(sessionInfo.getId());
		...
		grid.setRows(loanApprovalRegisterService.dataGrid(vo, ph));
		grid.setTotal(loanApprovalRegisterService.count(vo, ph));
		return grid;
	}
	...
	@ResponseBody
	public Json apply(LoanApprovalRegister obj, HttpServletRequest request) {
	...
	@RequestMapping("/add")
	@ResponseBody
	public Json add(LoanApprovalRegister obj, HttpServletRequest request) {
		...
		try {
			...
			// 业务表保存后主键
			Long id = loanApprovalRegisterService.add(obj, request);
			...
			loanApprovalRegisterService.update(obj);
	...
	@ResponseBody
	public Json commit(LoanApprovalRegister obj, HttpServletRequest request) {
		...
		try {
			...
			if (obj.getId() != null && obj.getId() > 0 && obj.getProcessId() != null) {
				...
				if (docId!= null && docId > 0) {
					...
					docId = loanApprovalRegisterService.add(obj,request);
				}
	...
	@RequestMapping("/edit")
	@ResponseBody
	public Json edit(LoanApprovalRegister obj, HttpServletRequest request) {
		Json j = new Json();
		try {
			loanApprovalRegisterService.update(obj);
			j.setSuccess(true);
	...
	public Json delete(String ids) {
		...
		for(String id : arr){
			LoanApprovalRegister obj = loanApprovalRegisterService.find(Long.valueOf(id));
	...
	public LoanApprovalRegister get(Long id) {
		LoanApprovalRegister obj = loanApprovalRegisterService.find(id);
		if(obj != null){
			Date d = obj.getLoanDate();
			if(d != null){
				String time = DateUtil.getDate(d);
				obj.setLoanDate_show(time);
			}
		}
		return obj;
	}
	...
	public String approvalDetailPage(HttpServletRequest request, Long id) {
		...
		LoanApprovalRegister obj = loanApprovalRegisterService.find(id);
		if (obj != null) {
			request.setAttribute("loanApprovalRegister", obj);
		}
		return "/basic/loanApprovalRegister/approvalDetail";
	}
	...
	public Json approval(Process process, HttpServletRequest request) {
		...
		// 审批通过
		LoanApprovalRegister obj = loanApprovalRegisterService.find(pro.getDocId());
		if(obj != null){
			Long userId = getSessionInfo(request).getId();
			User user = userService.get(userId);
			if (user != null) {
	 ...
	 * @param @param detail
	 */
	private void updateBusiness(LoanApprovalRegister obj, int stat) {
