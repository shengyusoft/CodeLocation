...
public class ScoreCutRegisterController extends BaseController {

	@Autowired
	private ScoreCutRegisterServiceI scoreCutRegisterService;
	...
	public Grid dataGrid(ScoreCutRegister vo, PageFilter ph, HttpServletRequest request) {
		...
		if (!StringUtils.isEmpty(user.getRoleNames())) {
			grid.setRows(scoreCutRegisterService.dataGrid(user, vo, ph));
			grid.setTotal(scoreCutRegisterService.count(user, vo, ph));
			return grid;
		}
		grid.setRows(scoreCutRegisterService.dataGrid(null, vo, ph));
	...
	public Json add(ScoreCutRegister obj, HttpServletRequest request) {
		...
		try {
			if (obj != null) {
				...
				// 申请人Id
				obj.setApplicantId(sessionInfo.getId());
				// 申请时间
	...
	@RequestMapping("/edit")
	@ResponseBody
	public Json edit(ScoreCutRegister obj, HttpServletRequest request) {
		Json j = new Json();
		try {
			scoreCutRegisterService.update(obj, request);
			j.setSuccess(true);
	...
	@RequestMapping("/get")
	@ResponseBody
	public ScoreCutRegister get(Long id) {
		ScoreCutRegister obj = scoreCutRegisterService.find(id);
		return obj;
	}
