...
<script type="text/javascript">
	...
	function editFun() {
		...
		parent.$.modalDialog({
			...
			height : '650',
			resizable : true,
			href : '${ctx}/purchasePlan/editPage?id=' + id,
			buttons : [
					/* {
						text : '编辑',
						handler : function() {
	...
	function detailFun() {
		...
		parent.$.modalDialog({
			...
			width : '815',
			height : '600',
			href : '${ctx}/purchasePlan/detailPage?id=' + id,
			buttons : [ {
				text : '退出',
				handler : function() {
...
<body class="easyui-layout" data-options="fit:true,bpurchasePlan:false">
	<div data-options="region:'center',fit:true,bpurchasePlan:false">
		<table id="dataGrid" data-options="fit:true,bpurchasePlan:false"></table>
	</div>
	<div id="toolbar" class="mygrid-toolbar" style="inline: true">

		<c:choose>
			<c:when
				test="${fn:contains(sessionInfo.resourceList, '/purchasePlan/add')}">
		...
		<c:choose>
			<c:when
				test="${fn:contains(sessionInfo.resourceList, '/purchasePlan/edit')}">
		...
		<c:choose>
			<c:when
				test="${fn:contains(sessionInfo.resourceList, '/purchasePlan/delete')}">
		...
		<c:choose>
			<c:when
				test="${fn:contains(sessionInfo.resourceList, '/purchasePlan/detail')}">
		...
		<c:if
			test="${fn:contains(sessionInfo.resourceList, '/purchasePlan/search')}">
