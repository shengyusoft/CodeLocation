...
<script type="text/javascript">
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${ctx}' + '/purchaseBalance/dataGrid',
			striped : true,
			rownumbers : true,
			pagination : true,
			nowrap : true,
	...
	function addFun() {
		parent.$.modalDialog({
			...
			height : '650',
			resizable : true,
			href : '${ctx}/purchaseBalance/addPage',
			buttons : [
					{
						id : 'qsave',
						text : '保存',
	...
	function deleteFun() {
		...
		parent.$.messager.confirm('询问', '确认删除选中的记录吗？', function(b) {
			if (b) {
				progressLoad();
				$.post('${ctx}/purchaseBalance/delete', {
					ids : selected
				}, function(result) {
					if (result.success) {
	...
	function editFun() {
		...
		parent.$.modalDialog({
			...
			height : '650',
			resizable : true,
			href : '${ctx}/purchaseBalance/editPage?id=' + id,
			buttons : [
					{
						text : '编辑',
						handler : function() {
	...
	function detailFun() {
		...
		parent.$.modalDialog({
			...
			width : '815',
			height : '600',
			href : '${ctx}/purchaseBalance/detailPage?id=' + id,
			buttons : [ {
				text : '退出',
				handler : function() {
...
<body class="easyui-layout" data-options="fit:true,bpurchaseBalance:false">
	<div data-options="region:'center',fit:true,bpurchaseBalance:false">
		<table id="dataGrid" data-options="fit:true,bpurchaseBalance:false"></table>
	</div>
	<div id="toolbar" class="mygrid-toolbar" style="inline: true">

		<c:choose>
			<c:when
				test="${fn:contains(sessionInfo.resourceList, '/purchaseBalance/add')}">
		...
		<c:choose>
			<c:when
				test="${fn:contains(sessionInfo.resourceList, '/purchaseBalance/edit')}">
		...
		<c:choose>
			<c:when
				test="${fn:contains(sessionInfo.resourceList, '/purchaseBalance/delete')}">
		...
		<c:choose>
			<c:when
				test="${fn:contains(sessionInfo.resourceList, '/purchaseBalance/detail')}">
		...
		<a onclick="" href="javascript:void(0);"
			class="easyui-linkbutton"
			data-options="plain:true,iconCls:'icon_toolbar_detail'">生成结算单 </a>

		<c:if
			test="${fn:contains(sessionInfo.resourceList, '/purchaseBalance/search')}">
