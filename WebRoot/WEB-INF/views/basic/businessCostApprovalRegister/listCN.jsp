...
<script type="text/javascript">
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${ctx}' + '/businessCostApprovalRegister/dataGrid?processFlag=12',
			striped : true,
			pagination : true,
			nowrap : true,
			idField : 'id',
	...
	function approvalCNFun() {
		...
		parent.$.modalDialog({
			...
			width : 800,
			height : 230,
			href : '${ctx}/businessCostApprovalRegister/approvalCNPage?id=' + id,
			buttons : [ {
				text : '编辑',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
					var f = parent.$.modalDialog.handler
							.find('#businessCostApprovalRegisterApprovalCNForm');
					f.submit();
				}
			} ]
		});
...
<body class="easyui-layout" data-options="fit:true,border:false">
	...
	<div id="toolbar" class="mygrid-toolbar" style="inline: true">
		...
		<c:choose>
			<c:when
				test="${fn:contains(sessionInfo.resourceList, '/businessCostApprovalRegister/approvalCN')}">
		...
		<c:if
			...
			<table style="width:900px;overflow: scroll;">
				<tr>
					...
					<th>办理时间:</th>
					<td>
						<input class="Wdate" type="text" name="handleDate" id="handleDate"
