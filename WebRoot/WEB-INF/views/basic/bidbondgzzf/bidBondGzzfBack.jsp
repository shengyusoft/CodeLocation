...
<script type="text/javascript">
	...
	function viewFun(viewType) {
		var me = this;
		var title = '', href = '${ctx}/bidBondGzzf/viewPage?type=1&viewType='+ viewType;
		var buttons = [];
		if ('add' == viewType) {
			title = '投标保证金退还申请登记';
			buttons = [ {
	...
	function submitForm() {
		...
		var f = parent.$.modalDialog.handler.find('#bidBondGzzfBackViewForm');
		f.submit();
	}
	...
	function printFun(type) {
		...
		id = rows[0].id;
		var url = ctxPath + "/report/bidBondGzzfBack?id=" + id+"&&type="+type;
		if(type == 0){
...
<body class="easyui-layout" data-options="fit:true,border:false">
	...
	<div id="toolbar" class="mygrid-toolbar" style="inline: true">
		...
		<c:if
			test="${fn:contains(sessionInfo.resourceList, '/bidBondGzzf/print')}">
