...
<script type="text/javascript">
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${ctx}' + '/loanApprovalRegister/dataGrid?processFlag=-10',
			striped : true,
			pagination : true,
			nowrap : true,
			idField : 'id',
	...
	function addFun() {
		parent.$.modalDialog({
			...
			width : 700,
			height : 350,
			href : '${ctx}/loanApprovalRegister/addPage',
			buttons : [ {
				text : '保存',
				id : 'loanApprovalRegisterBtn',
				handler : function() {
					//传入后台区分是保存还是申请
					parent.$.modalDialog.openner_dataGrid = dataGrid;
					parent.$.modalDialog.handler.find('#option').val(0);
					var f = parent.$.modalDialog.handler.find('#loanApprovalRegisterAddForm');
					f.submit();
				}
			},{
				text : '申请',
	...
	function editFun() {
		...
		parent.$.modalDialog({
			...
			width : 800,
			height : 320,
			href : '${ctx}/loanApprovalRegister/editPage?id=' + id,
			buttons : [ {
				text : '编辑',
				handler : function() {
	...
	function approvalFun() {
		...
		parent.$.modalDialog({
			...
			height : '520',
			resizable : true,
			href : ctxPath + '/loanApprovalRegister/approvalPage?id=' + id + '&processId=' + processId,
			buttons : [{
					text : '通过',
					handler : function () {
						var f = parent.$.modalDialog.handler.find('#loanApprovalRegisterApprovalForm');
						parent.$.modalDialog.openner_dataGrid = dataGrid;
						parent.$.modalDialog.handler.find('#option').val(0);
						f.submit();
					}
	...
	function approvalDetailFun() {
		...
		parent.$.modalDialog({
			...
			href : '${ctx}/loanApprovalRegister/approvalDetailPage?id=' + id,
			buttons : [ {
				text : '退出',
				handler : function() {
					//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
	...
	function printFun(type) {
		...
		var selected = getSelected();
		var url = ctxPath + "/report/loanApprovalRegister?type=" + type + "&ids=" + selected;
		console.log(url);
		if(type == 0){
			var tmp = window.open (
					url,
...
<body class="easyui-layout" data-options="fit:true,border:false">
	...
	<div id="toolbar" class="mygrid-toolbar" style="inline: true">
		...
		<c:choose>
			<c:when
				test="${fn:contains(sessionInfo.resourceList, '/loanApprovalRegister/approvalDetail')}">
		...
		<c:choose>
			<c:when
				test="${fn:contains(sessionInfo.resourceList, '/loanApprovalRegister/approval')}">
