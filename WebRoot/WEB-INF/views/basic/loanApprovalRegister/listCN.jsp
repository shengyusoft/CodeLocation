...
<script type="text/javascript">
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${ctx}' + '/loanApprovalRegister/dataGrid?processFlag=12',
			striped : true,
			pagination : true,
			nowrap : true,
			idField : 'id',
	...
	function approvalZJLFun() {
		...
		parent.$.modalDialog({
			...
			width : 700,
			height : 220,
			href : '${ctx}/loanApprovalRegister/approvalCNPage?id=' + id,
			buttons : [ {
				text : '提交',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
					var f = parent.$.modalDialog.handler
							.find('#loanApprovalRegisterApprovalCNForm');
					f.submit();
				}
			} ]
		});
...
<body class="easyui-layout" data-options="fit:true,border:false">
	...
	<div id="toolbar" class="mygrid-toolbar" style="inline: true">
		...
		<c:choose>
			<c:when
				test="${fn:contains(sessionInfo.resourceList, '/loanApprovalRegister/approvalCN')}">
		...
		<c:if
			...
			<table style="width:900px;overflow: scroll;">
				<tr>
					...
					<th>创建时间:</th>
					<td>
						<input class="Wdate" type="text" name="approveDate" id="approveDate" style="height: 100%"
