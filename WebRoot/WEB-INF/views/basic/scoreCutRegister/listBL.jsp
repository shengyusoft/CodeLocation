...
<script type="text/javascript">
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${ctx}' + '/scoreCutRegister/dataGrid?processFlag=23',
			striped : true,
			pagination : true,
			nowrap : true,
			idField : 'id',
	...
	function approvalBLFun() {
		...
		parent.$.modalDialog({
			...
			width : 800,
			height : 230,
			href : '${ctx}/scoreCutRegister/approvalBLPage?id=' + id,
			buttons : [ {
				text : '编辑',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
					var f = parent.$.modalDialog.handler
							.find('#scoreCutRegisterapprovalBLForm');
					f.submit();
				}
			} ]
		});
...
<body class="easyui-layout" data-options="fit:true,border:false">
	...
	<div id="toolbar" class="mygrid-toolbar" style="inline: true">
		...
		<c:choose>
			<c:when
				test="${fn:contains(sessionInfo.resourceList, '/scoreCutRegister/approvalBL')}">
				<a onclick="approvalBLFun();" href="javascript:void(0);"
