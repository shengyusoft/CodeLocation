...
<script type="text/javascript">
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${ctx}' + '/projectMeasureRegister/dataGrid',
			striped : true,
			pagination : true,
			nowrap : true,
			idField : 'id',
	...
	function addFun() {
		parent.$.modalDialog({
			...
			width : 750,
			height : 400,
			href : '${ctx}/projectMeasureRegister/addPage',
			buttons : [ {
				text : '添加',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个treeGrid，所以先预定义好
					var f = parent.$.modalDialog.handler
							.find('#projectMeasureRegisterAddForm');
					f.submit();
				}
			} ]
		});
	...
	function deleteFun() {
		...
		parent.$.messager.confirm('询问', '确认删除选中的记录吗？', function(b) {
			if (b) {
				progressLoad();
				$.post('${ctx}/projectMeasureRegister/delete', {
					ids : selected
				}, function(result) {
					if (result.success) {
	...
	function editFun() {
		...
		parent.$.modalDialog({
			...
			width : 750,
			height : 400,
			href : '${ctx}/projectMeasureRegister/editPage?id=' + id,
			buttons : [ {
				text : '编辑',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
					var f = parent.$.modalDialog.handler
							.find('#projectMeasureRegisterEditForm');
					f.submit();
				}
			} ]
		});
	...
	function detailFun() {
		...
		parent.$.modalDialog({
			...
			width : 750,
			height : 400,
			href : '${ctx}/projectMeasureRegister/detailPage?id=' + id,
			buttons : [ {
				text : '退出',
				handler : function() {
...
<body class="easyui-layout" data-options="fit:true,border:false">
	...
	<div id="toolbar" class="mygrid-toolbar" style="inline: true">
		...
		<c:choose>
			<c:when
				test="${fn:contains(sessionInfo.resourceList, '/projectMeasureRegister/add')}">
		...
		<c:choose>
			<c:when
				test="${fn:contains(sessionInfo.resourceList, '/projectMeasureRegister/edit')}">
		...
		<c:choose>
			<c:when
				test="${fn:contains(sessionInfo.resourceList, '/projectMeasureRegister/delete')}">
		...
		<c:choose>
			<c:when
				test="${fn:contains(sessionInfo.resourceList, '/projectMeasureRegister/detail')}">
		...
		<c:if
			test="${fn:contains(sessionInfo.resourceList, '/projectMeasureRegister/search')}">
