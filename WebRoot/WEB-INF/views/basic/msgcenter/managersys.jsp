...
$(function() {
	dataGrid = $('#dataGrid').datagrid({
				url : ctxPath + '/managersys/dataGrid',
				striped : true,
				rownumbers : true,
				pagination : true,
				nowrap : true,
...
function addFun() {
	parent.$.modalDialog({
		...
		width : 715,
		height : 600,
		href : ctxPath + '/managersys/addPage',
		buttons : [ {
			text : '添加',
			handler : function() {
				parent.$.modalDialog.openner_dataGrid = dataGrid;// 因为添加成功之后，需要刷新这个treeGrid，所以先预定义好
				var f = parent.$.modalDialog.handler.find('#managersysAddForm');
				f.submit();
			}
		} ]
	});
...
function deleteFun() {
	...
	parent.$.messager.confirm('询问', '确认删除选中的记录吗？', function(b) {
		if (b) {
			progressLoad();
			$.post(ctxPath + '/managersys/delete', {
				ids : selected
			}, function(result) {
				if (result.success) {
...
function editFun() {
	...
	parent.$.modalDialog({
		...
		width : 715,
		height : 600,
		href : ctxPath + '/managersys/editPage?id=' + id,
		buttons : [ {
			text : '编辑',
			handler : function() {
				parent.$.modalDialog.openner_dataGrid = dataGrid;// 因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
				var f = parent.$.modalDialog.handler.find('#managersysEditForm');
				f.submit();
			}
		} ]
	});
...
function detailFun() {
	...
	$.ajax({
		...
		success : function(article) {
			...
			addTab({
				url : ctxPath + '/managersys/detailPage?id=' + id,
				title : title + '详情',
				background : '#F0F5FB'
			});
		},
...
<body class="easyui-layout" data-options="fit:true,border:false">
	...
	<div id="toolbar" class="mygrid-toolbar" style="inline:true">
		...
		<c:choose>
			<c:when test="${fn:contains(sessionInfo.resourceList, '/managersys/add')}">
		...
		<c:choose>
			<c:when test="${fn:contains(sessionInfo.resourceList, '/managersys/edit')}">
		...
		<c:choose>
			<c:when test="${fn:contains(sessionInfo.resourceList, '/managersys/delete')}">
		...
		<c:choose>
			<c:when test="${fn:contains(sessionInfo.resourceList, '/managersys/detail')}">
		...
		<c:if test="${fn:contains(sessionInfo.resourceList, '/managersys/search')}">
