...
$(function() {
		...
		url : ctxPath + '/knowledge/dataGrid',
		striped : true,
		rownumbers : true,
		pagination : true,
		nowrap : true,
...
function addFun() {
	parent.$.modalDialog({
		...
		width : 715,
		height : 600,
		href : ctxPath + '/knowledge/addPage',
		buttons : [ {
			text : '添加',
			handler : function() {
...
function editFun() {
	...
	parent.$.modalDialog({
		...
		width : 715,
		height : 600,
		href : ctxPath + '/knowledge/editPage?id=' + id,
		buttons : [ {
			text : '编辑',
			handler : function() {
...
function detailFun() {
	...
	$.ajax({
		...
		success : function(article) {
			...
			addTab({
				url : ctxPath + '/knowledge/detailPage?id=' + id,
				title : title + '详情',
				background : '#F0F5FB'
			});
		},
...
<body class="easyui-layout" data-options="fit:true,border:false">
	...
	<div id="toolbar" class="mygrid-toolbar" style="inline:true">
		...
		<c:choose>
			<c:when test="${fn:contains(sessionInfo.resourceList, '/knowledge/add')}">
		...
		<c:choose>
			<c:when test="${fn:contains(sessionInfo.resourceList, '/knowledge/edit')}">
		...
		<c:choose>
			<c:when test="${fn:contains(sessionInfo.resourceList, '/knowledge/delete')}">
		...
		<c:choose>
			<c:when test="${fn:contains(sessionInfo.resourceList, '/knowledge/detail')}">
		...
		<c:if test="${fn:contains(sessionInfo.resourceList, '/knowledge/search')}">
