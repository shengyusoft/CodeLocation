...
<script type="text/javascript">
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${ctx}' + '/payrollRegister/dataGrid?processFlag=-20',
			striped : true,
			pagination : true,
			nowrap : true,
			idField : 'id',
	...
	function approvalZJLFun() {
		...
		parent.$.modalDialog({
			...
			width : 800,
			height : 320,
			href : '${ctx}/payrollRegister/approvalZJLPage?id=' + id,
			buttons : [ {
				text : '编辑',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
					var f = parent.$.modalDialog.handler
							.find('#payrollRegisterApprovalZJLForm');
					f.submit();
				}
			} ]
		});
...
<body class="easyui-layout" data-options="fit:true,border:false">
	...
	<div id="toolbar" class="mygrid-toolbar" style="inline: true">
		...
		<c:choose>
			<c:when
				test="${fn:contains(sessionInfo.resourceList, '/payrollRegister/approvalZJL')}">
