...
<script type="text/javascript">
	...
	function initProcessImg(){
		...
		if(state <= 0){
			//申请
			$('#process')[0].style.left = '54px';
			$('#process')[0].style.top = '66px';
			$('#process')[0].style.height = '64px';
			$('#process')[0].style.width = '114px';
		}else if(state == 1){
			//综合部
			$('#process')[0].style.left = '226px';
			$('#process')[0].style.top = '66px';
			$('#process')[0].style.height = '63px';
			$('#process')[0].style.width = '112px';
		}else if(state == 2){
			//结束
			$('#process')[0].style.left = '532px';
			$('#process')[0].style.top = '72px';
...
<div class="easyui-layout" data-options="fit:true,border:false">
	<div data-options="region:'center',fit:true,collapsed:false"
		...
		<div title="流程图"
			...
			<img src="${ctx}/style/images/flow/flow_zc.png" />
