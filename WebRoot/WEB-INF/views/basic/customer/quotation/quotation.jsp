...
<script type="text/javascript">
	...
	function editFun() {
		...
		parent.$.modalDialog({
			...
			height : '650',
			resizable : true,
			href : '${ctx}/quotation/editPage?id=' + id,
			buttons : [
					{
						text : '编辑',
						handler : function() {
	...
	function detailFun() {
		...
		parent.$.modalDialog({
			...
			width : '815',
			height : '600',
			href : '${ctx}/quotation/detailPage?id=' + id,
			buttons : [ {
				text : '退出',
				handler : function() {
...
<body class="easyui-layout" data-options="fit:true,border:false">
	...
	<div id="toolbar" class="mygrid-toolbar" style="inline: true">
		...
		<c:choose>
			<c:when
				test="${fn:contains(sessionInfo.resourceList, '/quotation/add')}">
		...
		<c:choose>
			<c:when
				test="${fn:contains(sessionInfo.resourceList, '/quotation/edit')}">
		...
		<c:choose>
			<c:when
				test="${fn:contains(sessionInfo.resourceList, '/quotation/delete')}">
		...
		<c:choose>
			<c:when
				test="${fn:contains(sessionInfo.resourceList, '/quotation/detail')}">
		...
		<a onclick="generateOrder();" href="javascript:void(0);"
			class="easyui-linkbutton"
			data-options="plain:true,iconCls:'icon_toolbar_detail'">生成订单 </a>

		<c:if
			test="${fn:contains(sessionInfo.resourceList, '/quotation/search')}">
